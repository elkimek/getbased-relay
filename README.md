...
- actual serialized pending envelopes plus acknowledgement receipts are bounded by a context-state-aware owner partition that preserves existing contexts plus one valid maximum-size context add or replacement; retained proposal IDs are additionally bounded per token and owner-wide.
...
Proposal limits are configured with `AGENT_PROPOSAL_MAX_CIPHERTEXT_BYTES` (default 64 KiB ciphertext), `AGENT_PROPOSAL_MAX_PENDING` (default 20 pending per token), `AGENT_PROPOSAL_MAX_TRACKED` (default 256 pending plus acknowledged IDs per token), and `AGENT_PROPOSAL_RETENTION_MS` (default 24 hours). The relay also derives an owner-wide serialized proposal-storage partition from current context sizes and profile capacity, so neither receipt cycling nor large pending ciphertext can consume the bytes required to preserve existing contexts plus one valid maximum-size context add or replacement.
