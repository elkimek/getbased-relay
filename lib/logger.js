...
export function createLogger(config) {
  ...
  let _onOwnerSeen = null; // set via setOwnerCallback
  ...
  function parseRelayLog(method, args) {
    ...
    if (args[0] === '[relay]') {
      ...
      // Track owner activity from subscribe events
      if (tag === 'subscribe' && data?.ownerId && _onOwnerSeen) {
        _onOwnerSeen(data.ownerId);
      }
  ...
  return {
    ...
    setOwnerCallback: (fn) => { _onOwnerSeen = fn; },
  };
}
